...
std::vector<byte> AES::_expandKey(const std::vector<byte>& cipherKey, bool verbose)
{
	...
	std::vector<byte> roundKey(this->Nstate_ * (this->Nround_ + 1), 0);  //Nstate_ -> 16   Nround_-> 10   176

	for (uint8_t i = 0; i < this->Nkey_; i++)  //Nkey_  -> 4
	{
	...
	for (uint8_t i = this->Nkey_; i < this->Ncol_ * (this->Nround_ + 1); i++)       // for 4 to  44
	{
		byte Wi_1[4];
		uint8_t i_1 = (i - 1) * 4;
		...
		if (i % this->Nkey_ == 0) {
			...
			// XOR Rcon
			Wi_1[0] = Wi_1[0] ^ rcon_[i / this->Nkey_];   //rcon_[11] ={ 0xFF, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36}
		}
	...
	if (verbose) {
		for (uint8_t i = 0; i < this->Nround_ + 1; i++) {                // Nround_-> 10
			printf("\n----- Round Key %d ----\n", i);

			for (uint8_t r = 0; r < this->Bsize_; r++)      // Bsize_  -> 4
			{
				printf("      ");
...
{
	this->_addRoundKey(state, roundKey, 0, verbose);  //라운드 키 추가 
...
{
	// Nkey_  -> 128 / 32    = 4  - 초기화에서 
	// Nround_ ->  Nkey(4) + 6 = 10
	//cipherKey.size()   Nkey_ * 4 * 2 -> 32
	assert(cipherKey.size() == this->Nkey_ * 4 * 2); //  암호화키가 32문자열이 맞는지 확인  check cipherKey(str) size  3 == 128 / 192 / 256 bits * 2

	uint32_t block_size = input.length() / (this->Nstate_ * 2);  // 32byte로 블럭사이즈 만듦     //number of state matrix's bytes   16*2
	if (input.length() % (this->Nstate_ * 2) != 0)
		block_size += 1;                         // 길이가 남으면 블럭을 하나 더

	std::string cipherText = "";
	std::vector<byte> plainText = this->_convertTypeStrToByteBlock(input);
	printf("\n----- plainText -----\n");
	this->_printState(plainText);
	std::vector<byte> _cipherKey = this->_convertTypeStrToByteBlock(cipherKey);
	printf("\n----- _cipherKey -----\n");
	this->_printState(_cipherKey);

	std::vector<byte> roundKey = this->_expandKey(_cipherKey, verbose);    //roundKey size 176  16 * 11
	...
	for (uint32_t i = 0; i < block_size; i++) {
		std::vector<byte> state = this->_createState(plainText, i * this->Nstate_);  //Nstate_ -> 16
